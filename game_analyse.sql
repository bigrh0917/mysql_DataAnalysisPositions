...
create view v_game_clean_workplace as
with process as (select *,
                        (case
...
#处理职位名称
create view v_game_clean_jobname as
select *
from v_game_clean_workplace
where job_name like '%游戏%';


#5420
select count(*)
from v_game_clean_jobname;

#对清理结果命名

create view v_game_clean as
(
select *
from v_game_clean_jobname);
...
create view v_game_market_demand as
...
#就业企业类型分布
create view v_game_companytype_degree as
(
...
from (select companytype_text,
     ...
     (select sum(degreefrom) as sum_degreefrom from v_game_clean) f2);

#查询结果
select *
from v_game_companytype_degree;

#规范薪资，单位
#千/月->1000
#万/月->10000
#万/年->833
create view v_game_salary_unit as
select *,
       (case
...
create view v_game_salary_min_max_mean as
with process as (select *,
                        (case
                             when unit = 1000 then
                                     cast(substring_index(
...
#按照工作年限分组，求各组平均薪资
create view v_game_workyear_salary as
select workyear         as '工作年限',
       avg(salary_mean) as '平均薪资'
from v_game_salary_min_max_mean
group by workyear
order by length(workyear), workyear;

#查询结果
select *
from v_game_workyear_salary;

#按企业类型分组，计算平均薪资
create view v_game_companytype_salary as
select companytype_text as '企业类型',
       avg(salary_mean) as '平均薪资'
from v_game_salary_min_max_mean
group by companytype_text
...
select *
from v_game_companytype_salary;
...
#得到招聘对应的技能需求
create view v_game_skill_quantity as
select skill,
       count(*) as quantity
from skill_table
         inner join v_game_clean
                    on v_game_clean.parse2_job_detail like concat('%', skill_table.skill, '%')
...
#技能出现的频数
create view v_game_skill as
select skill                                                               as '技能',
       quantity                                                            as '出现频数',
       concat(cast(quantity / total_quantity * 100 as decimal(4, 2)), '%') as '出现率'
from v_game_skill_quantity as f1,
     (select count(*) as total_quantity from v_game_clean) f2;
select *
from v_game_skill;
